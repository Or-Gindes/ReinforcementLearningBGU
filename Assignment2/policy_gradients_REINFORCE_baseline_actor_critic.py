    ...
    def __init__(self, state_size, action_size, learning_rate, discount_factor=0.99, name='policy_network',
        ...
        with tf.variable_scope(name):
            ...
            if baseline:
                ...
                self.advantage = self.R_t - self.baseline.state_output
                self.loss = tf.reduce_mean(self.neg_log_prob * (self.R_t - tf.stop_gradient(self.baseline.state_output)))
...
def run():
    ...
    policy = PolicyNetwork(state_size, action_size, learning_rate, discount_factor=discount_factor,
                           baseline=True if method == "baseline" else False,
                           critic=True if method == "critic" else False)
    # Start training the agent with REINFORCE w/baseline algorithm
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        writer = tf.summary.FileWriter(f"./logs/REINFORCE_{method}", sess.graph)
        solved = False
