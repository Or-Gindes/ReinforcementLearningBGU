...
TRANSFER_SCENARIO1 = {"source": "Acrobot-v1", "target": "CartPole-v1"}
TRANSFER_SCENARIO2 = {"source": "CartPole-v1", "target": "MountainCarContinuous-v0"}
...
def fine_tune(source_env_name, target_env_name):
    ...
    policy = ActorCritic(STANDARDIZED_STATE_SIZE, STANDARDIZED_ACTION_SIZE, LEARNING_RATE, DISCOUNT_FACTOR)
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter(f"./logs/section2/{source_env_name}", sess.graph)
        solved = False
        episode_rewards = np.zeros(MAX_EPISODES)
        average_rewards = -1e3

        if target_env_name == 'MountainCarContinuous-v0':
            num_goal_reached = 0
            scaler = scale_observations(target_env, STANDARDIZED_ACTION_SIZE)
